...
var utils = module.exports = {
  ...
  isOptionsHash: function (o) {
    return _.isPlainObject(o) && _.some(['api_key', 'idempotency_key', 'baabedo_account'], function(key) {
  ...
  getOptionsFromArgs: function(args) {
    ...
    if (args.length > 0) {
      ...
      if (utils.isAuthKey(arg)) {
        opts.auth = args.pop();
        opts.query = {};
        opts.query['access_token'] = opts.auth;
        ...
        for (var key in params) {
          if (params.hasOwnProperty(key)) {
            if (key == 'api_key') {
              opts.auth = params[key];
              opts.query['access_token'] = opts.auth;
            }
            else if (key == 'idempotency_key') {
              opts.headers['Idempotency-Key'] = params[key];
            }
            else if (key == 'baabedo_account') {
              opts.headers['Baabedo-Account'] = params[key];
            }
            else {
              opts.query[key] = params[key];
            }
          }
        }
      }
